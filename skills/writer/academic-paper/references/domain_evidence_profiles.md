...
The substance of the field-centric `## Field-Specific Adjustments` table in `skills/surveyor/deep-research/references/source_quality_hierarchy.md` is carried forward here so no per-field guidance is silently dropped. The deep-research file is **read, not edited** — this is a one-time authoring copy, not a runtime dual-read.
