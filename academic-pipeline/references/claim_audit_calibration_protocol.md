...
- Parent agent: `skills/orchestrator/academic-pipeline/agents/claim_ref_alignment_audit_agent.md` — dispatches calibration mode and consumes the report's thresholds block. The agent prompt and this doc form a two-way pair following the v3.6.5 protocol-doc convention (`literature_corpus_consumers.md` ↔ `bibliography_agent.md`).
- Reviewer calibration baseline: `skills/critic/academic-paper-reviewer/references/calibration_mode_protocol.md` (same vocabulary, different unit of analysis).
