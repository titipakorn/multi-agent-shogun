...
# ============================================================
# v2 specialist team — emit per-CLI variants of specialist prompts
# ============================================================
...
# CLAUDE.md is the source of truth, and is generated by replacing Claude-specific parts with Codex-specific ones.
generate_agents_md() {
    ...
    sed \
        ...
        -e 's|sends `/clear` + Enter via send-keys|sends `/new` + Enter via send-keys (auto-converted /clear -> /new)|g' \
        ...
        -e 's|delivers `/clear` to the agent|delivers `/new` to the agent (auto-converted /clear -> /new)|g' \
...
# ============================================================
# GitHub Copilot CLI automatically loads .github/copilot-instructions.md.
# CLAUDE.md is the source of truth, and is generated by replacing Claude-specific parts with Copilot-specific ones.
...
# ============================================================
# Kimi K2 CLI automatically loads agents/default/agent.yaml + system.md.
# CLAUDE.md is the source of truth, and is generated by replacing Claude-specific parts with Kimi-specific ones.
...
mode: primary
# Auto-generated by build_instructions.sh — do not edit manually.
# Source: instructions/${role}.md (v2 specialists are self-contained) OR
#         instructions/roles/${role}_role.md + instructions/common/* (canonical 3)
...
FRONTMATTER
        ...
        # Append role-specific content
        # Canonical 3 (shogun, orchestrator, telegram) use the legacy template
        # at instructions/roles/<role>_role.md concatenated with common sections.
        ...
        case "$agent_id" in
            surveyor|critic|architect|experimentalist|analyst|ablation_planner|writer|observer|council)
                if [ -f "$ROOT_DIR/instructions/${agent_id}.md" ]; then
                    ...
                    echo "  ⚠️  instructions/${agent_id}.md not found; skipping ${agent_id}" >&2
                    continue
                fi
                ;;
            *)
                role_body="$PARTS_DIR/roles/${role}_role.md"
                ;;
        esac
        {
            cat "$role_body"
