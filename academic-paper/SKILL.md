...
> **Routing discipline (v3.9.2):** see `CLAUDE.md` "bloom_routing_rule" + `shared/references/intent_clarification_protocol.md` for cross-skill routing rules. This skill assumes routing has already settled — ambiguous cross-phase materials should have been clarified upstream.
...
Routing into Mode B requires explicit user signal — `/ars-<mode>` slash command or `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per `CLAUDE.md` "bloom_routing_rule" + `shared/references/intent_clarification_protocol.md`.
...
v3.6.6 splits Phase 4 (writer drafting) and Phase 6 (in-pair evaluator review) into paper-blind / paper-visible call pairs gated by the `writer_full` and `evaluator_full` contracts. The split mirrors `skills/critic/academic-paper-reviewer/references/sprint_contract_protocol.md` (the v3.6.2 reviewer pattern) but adapts it for single-agent generator modes that have no panel and (for the writer) no scoring_plan.
...
`intake_agent` automatically detects deep-research materials (RQ Brief / Bibliography / Synthesis / INSIGHT Collection) and skips redundant steps. See `skills/surveyor/deep-research/SKILL.md` Handoff Protocol for the complete handoff material format.
...
See `skills/orchestrator/academic-pipeline/SKILL.md` for the complete workflow.
...
- Also: `skills/surveyor/deep-research/references/apa7_style_guide.md` (base reference, extended here)
...
| 5 | **⚠️ IRON RULE: Fabricated citations** | Inventing plausible-sounding references that don't exist | Every citation must be verified via DOI or WebSearch; see `skills/orchestrator/academic-pipeline/agents/integrity_verification_agent.md` |
